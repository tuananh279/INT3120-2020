...
export default class GreetingScreen extends Component {
  ...
  listenForItems(itemRef) {
    var items = [];
    this.itemRef.ref('data').on('child_added', (dataSnapshot) => {
      items.push({
  ...
  removeDB(rowData){
    this.itemRef.ref('data').child(rowData._key).remove();
    this.listenForItems(this.itemRef);
    alert('Đã xóa khỏi Yêu thích')
  }
  showDB() {
    this.itemRef.ref('favority').child(this.state.text).on('value', function(snapshot){
      alert(snapshot.val())
    });
        ...
        renderRow = {(rowData) => 
          <View style={styles.item}>
            <View style={({flex: 1, width: '20%', height: '100%'}, styles.canchinh1)}>
              <TouchableOpacity onPress={() => tts.speak(rowData.name)}>
            ...
            <View style={{flex: 3,height: '100%', justifyContent:'center', alignItems: 'center', width: '60%'}}>            
              <Text style={{fontSize: 20, fontWeight: '500'}} >{rowData.name}</Text>
            ...
            <TouchableOpacity onPress={() => this.removeDB(rowData)}>
               <FontAwesome5 name={'trash'} size={30} color={'#FF9D00'} />
               </TouchableOpacity>
            </View>
          </View>
        }>
