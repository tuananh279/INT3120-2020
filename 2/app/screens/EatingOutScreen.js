...
export default class EatingOutScreen extends Component {
  constructor(props) {
    super(props);
  ...
  listenForItems(itemRef) {
    var items = [];
    this.itemRef.ref('eating').on('child_added', (dataSnapshot) => {
      items.push({
  ...
  showDB() {
    this.itemRef.ref('eating').child(this.state.text).on('value', function(snapshot){
      alert(snapshot.val())
    });
