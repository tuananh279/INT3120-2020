...
export default class MedicalExaminationScreen extends Component {
  constructor(props) {
    super(props);
  ...
  listenForItems(itemRef) {
    var items = [];
    this.itemRef.ref('medical').on('child_added', (dataSnapshot) => {
      items.push({
  ...
  showDB() {
    this.itemRef.ref('medical').child(this.state.text).on('value', function(snapshot){
      alert(snapshot.val())
    });
