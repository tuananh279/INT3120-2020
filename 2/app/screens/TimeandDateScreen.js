...
export default class TimeandDateScreen extends Component {
  constructor(props) {
    super(props);
  ...
  listenForItems(itemRef) {
    var items = [];
    this.itemRef.ref('timeanddate').on('child_added', (dataSnapshot) => {
      items.push({
  ...
  showDB() {
    this.itemRef.ref('timeanddate').child(this.state.text).on('value', function(snapshot){
      alert(snapshot.val())
    });
