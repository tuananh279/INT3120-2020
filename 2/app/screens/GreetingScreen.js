...
export default class GreetingScreen extends Component {
  ...
  listenForItems(itemRef) {
    var items = [];
    this.itemRef.ref('word').on('child_added', (dataSnapshot) => {
      items.push({
  ...
  showDB() {
    this.itemRef.ref('words').child(this.state.text).on('value', function(snapshot){
      alert(snapshot.val())
    });
  ...
  // playAudio() {
  //   var whoosh = new Sound('https://firebasestorage.googleapis.com/v0/b/awabeclone.appspot.com/o/hello_en_us_1.mp3?alt=media&token=37658bde-d02d-4717-8c31-a97d21da8970', Sound.MAIN_BUNDLE, (error) => {
  //     if (error) {
