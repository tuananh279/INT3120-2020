...
export default class DatingScreen extends Component {
  constructor(props) {
    super(props);
  ...
  listenForItems(itemRef) {
    var items = [];
    this.itemRef.ref('dating').on('child_added', (dataSnapshot) => {
      items.push({
  ...
  showDB() {
    this.itemRef.ref('dating').child(this.state.text).on('value', function(snapshot){
      alert(snapshot.val())
    });
